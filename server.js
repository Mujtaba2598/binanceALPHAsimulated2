...
async function startSimulatedTrading(sessionId) {
    ...
    // Process existing simulated trades
    ...
    // If target reached after completions
    if (session.currentBalance >= session.targetAmount) {
        session.status = 'TARGET_REACHED';
        updateSimulatedBalance(session.userId, session.currentBalance);
        activeSessions.delete(sessionId);
    ...
    // Calculate remaining needed and time remaining
    ...
    // Determine number of trades to place
    ...
    if (investmentPerTrade < 10 && session.currentBalance < 10) {
    ...
    for (let i = 0; i < tradesToPlace; i++) {
        ...
        newTradesPlaced++;
        console.log(`📊 NEW ORDER: $${investmentPerTrade.toFixed(2)} → ${roundedQty} ${symbol} at $${buyPrice} (Target: ${session.profitTargetPercent}%) | Real price: $${currentPrice}`);
    }
